...
const columns: ColumnDef<z.infer<typeof orderSchema>>[] = [
    ...
    },
    {
        ...
        header: "Invoice ID",
        cell: ({ row }) => (
            <div className="text-left">{row.original.invoiceId}</div>
        ),
    },
    {
        accessorKey: "status",
        ...
        cell: ({ row }) => (
            <Badge variant="outline" className="text-muted-foreground px-1.5">
                {row.original.status === OrderStatus.Paid && (
                ...
                {row.original.status === OrderStatus.Finished && (
            ...
            >
                ...
                <div className="flex items-center justify-between px-4">
                    <div className="text-muted-foreground hidden flex-1 text-sm lg:flex">
                        There are {table.getFilteredRowModel().rows.length}{" "}
                        row(s)
                    </div>
...
function TableCellViewer({ item }: { item: z.infer<typeof orderSchema> }) {
    ...
    return (
        <Link
            href={"#"}
            className="text-foreground w-fit px-0 text-left hover:underline underline-offset-4"
        >
            {item.id}
        </Link>
    );
